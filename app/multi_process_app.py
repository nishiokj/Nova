...
from audio import AudioDeviceManager, AudioConfig as DeviceAudioConfig
from harness import ProcessManager, create_agent_worker, create_tts_worker
...
class MultiProcessVoiceApp(BaseVoiceApp):
    ...
    def __init__(self, config: AppConfig):
        ...
        self.event_bus = MultiProcessEventBus(
            max_agent_pending=config.runtime.max_agent_pending
        )

        # Create process manager
        self.process_manager = ProcessManager(
            event_bus=self.event_bus._harness_bus,  # Use underlying harness bus
            check_interval=config.runtime.health_check_interval_s,
            agent_timeout=config.runtime.agent_timeout_s,
            tts_timeout=config.runtime.tts_timeout_s
        )
        ...
        try:
            ...
            while self.running:
                try:
                    ...
                    if self.event_bus.tts_speaking_event.is_set():
                        time.sleep(0.1)
                        continue
    ...
    def _audio_process_loop(self):
        ...
        while self.running:
            try:
                ...
                if audio_result:
                    # Transcribe
                    from services.audio_service import AudioChunk
                    ...
                    if stt_result and stt_result.text:
                        ...
                        # Submit to Agent process via EventBus
                        self._request_count += 1
                        request_id = f"req_{self._request_count}_{self._generate_request_id()[:8]}"

                        self.logger.info(f"[{request_id}] Submitting: {lint_result.cleaned}")

                        self.event_bus.publish(AgentRequestSubmittedEvent(
                            request_id=request_id,
                            speech_text=lint_result.cleaned,
                            tier=self.config.harness.default_tier,
                            context=None,
    ...
    def _response_monitor_loop(self):
        ...
        while self.running:
            try:
                result = self.event_bus.get_event(timeout=0.5)
                if result:
                    self.logger.info(
                        f"[{result.request_id}] Response in {result.duration_ms:.0f}ms: "
    ...
    def start(self) -> bool:
        ...
        self.process_manager.set_tts_factory(
            create_tts_worker(self.event_bus._harness_bus, voice_config)
        )

        self.process_manager.set_agent_factory(
            create_agent_worker(
                self.event_bus._harness_bus,
                config=None,  # Will load from config_path
