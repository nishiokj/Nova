...
```typescript
// packages/graphd/src/types.ts - ENRICHED
...
// packages/work/src/work-item.ts - NO CHANGES
interface WorkItem {
  workId: string;
...
|----------|---------|-------------|
| `packages/decision-watcher/src/watcher-agent.ts` | `handlePromptUser` returning `action: 'escalate'` | Watcher decides question needs human input |
| `packages/decision-watcher/src/watcher-agent.ts` | `handleCadenceAudit` detecting uncertainty | Periodic audit finds agent stuck or drifting |
| `packages/decision-watcher/src/watcher-agent.ts` | `handleAgentError` unrecoverable | Error that can't be retried |
| `packages/decision-watcher/src/watcher-agent.ts` | `handleWorkItemCompleted` quality gate needs human | QualityGateDecision with `verdict: 'needs_human'` |
| `packages/orchestrator/src/orchestrator.ts` | Resource bounds hit with no watcher resolution | Escalate to human for guidance |
...
```typescript
// packages/agent/src/types.ts - ADD to InternalHookEvent union
...
```typescript
// packages/protocol/src/domain/events.ts - ADD to ControlEvent union
...
```typescript
// packages/protocol/src/control/decisions.ts - ADD
...
| Type | Package | File |
|------|---------|------|
| `SessionStatus` (enriched) | `packages/graphd` | `src/types.ts` |
| `Escalation`, `EscalationType`, `EscalationStatus`, `EscalationOption`, `EscalationReference` | `packages/types` | `src/escalation.ts` (NEW) |
| `EscalationRaisedEvent`, `EscalationResolvedEvent`, `SessionStatusChangedEvent` (InternalHookEvent) | `packages/agent` | `src/types.ts` |
| `EscalationResolvedEvent` (ControlEvent) | `packages/protocol` | `src/domain/events.ts` |
| `EscalationResolutionDecision` | `packages/protocol` | `src/control/decisions.ts` |
| Zod schemas | `packages/types` | `src/schemas/escalation.ts` (NEW) |
...
2. **Create Escalation types** — Add to `packages/types/`
3. **Create escalations table** — Migration in `packages/agent-memory/`
4. **Extend InternalHookEvent** — Add escalation events to `packages/agent/src/types.ts`
5. **Extend ControlEvent** — Add `EscalationResolvedEvent` to `packages/protocol/`
