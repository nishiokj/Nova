/**
 * Shared bridge command and event types for Nova clients.
 */
...
export const BRIDGE_EVENT_TYPES: ReadonlySet<BridgeEventType> = new Set<BridgeEventType>([
  'ready',
  'status',
  'progress',
  'stream',
...
export const BRIDGE_COMMAND_TYPES: ReadonlySet<BridgeCommandType> = new Set<BridgeCommandType>([
  'init',
  'send_text',
  'send_media',
  'user_prompt_response',
...
export function isBridgeCommand(payload: unknown): payload is BridgeCommand {
  if (!payload || typeof payload !== 'object') return false;
  const p = payload as Record<string, unknown>;

  if (typeof p.type !== 'string' || !BRIDGE_COMMAND_TYPES.has(p.type as BridgeCommandType)) {
    return false;
  }

  return p.data === undefined || (typeof p.data === 'object' && p.data !== null);
}
...
export function isBridgeEvent(payload: unknown): payload is BridgeEvent {
  ...
  if (typeof p.type !== 'string' || !BRIDGE_EVENT_TYPES.has(p.type as BridgeEventType)) {
    return false;
  }
