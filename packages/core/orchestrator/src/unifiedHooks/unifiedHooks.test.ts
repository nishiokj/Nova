...
import type { UnifiedDecisionHookRegistration, UnifiedEffectHookRegistration } from './contracts.js';
import { createSessionScopedUnifiedHookRegistry, createUnifiedHookRegistry } from './registry.js';
import {
...
describe('unified hooks registry', () => {
  it('registers decision and effect hooks with deterministic ordering', () => {
    ...
    const decision: UnifiedDecisionHookRegistration<'cadence_audit'> = {
      ...
      callback: async () => success({ action: 'continue' }),
    };
...
describe('unified decision runner', () => {
  it('returns the first decision by priority and registration order', async () => {
    ...
    const early: UnifiedDecisionHookRegistration<'cadence_audit'> = {
      ...
      criticality: 'non_critical',
      idempotency: 'idempotent',
      callback: async () => success({ action: 'inject_guidance', message: 'use tests' }),
    };
    ...
    const result = await runUnifiedDecisionHooks(createCadenceEvent(), createHookContext(), registry);
...
describe('unified effect runner', () => {
  it('supports lifecycle block outcomes for blockable events', async () => {
    ...
    const blocker: UnifiedEffectHookRegistration<'pre_tool_use'> = {
      ...
      timeoutMs: 100,
      policy: fireAndForget(),
      callback: async () => ({ kind: 'block', reason: 'policy violation' }),
    };

    registry.register(blocker);

    const result = await runUnifiedEffectHooks(
      {
        type: 'pre_tool_use',
...
describe('session-scoped unified hooks', () => {
  it('isolates registration by session and allows same hook id across sessions', async () => {
    ...
    const hookA: UnifiedDecisionHookRegistration<'cadence_audit'> = {
      ...
      criticality: 'non_critical',
      idempotency: 'idempotent',
      callback: async () => success({ action: 'inject_guidance', message: 'from session A' }),
    };
    ...
    const hookB: UnifiedDecisionHookRegistration<'cadence_audit'> = {
      ...
      criticality: 'non_critical',
      idempotency: 'idempotent',
      callback: async () => success({ action: 'inject_guidance', message: 'from session B' }),
    };

    sessionRegistry.register('session-a', hookA);
    sessionRegistry.register('session-b', hookB);

    const resultA = await runUnifiedDecisionHooksForSession(
      'session-a',
      createCadenceEvent(),
      createHookContext(),
      sessionRegistry
    ...
    const resultB = await runUnifiedDecisionHooksForSession(
      'session-b',
      createCadenceEvent(),
      createHookContext(),
      sessionRegistry
  ...
  it('returns no hooks for sessions without registrations', async () => {
    ...
    const decisionResult = await runUnifiedDecisionHooksForSession(
      'missing-session',
      createCadenceEvent(),
      createHookContext(),
      sessionRegistry
    ...
    const effectResult = await runUnifiedEffectHooksForSession(
      'missing-session',
      {
        type: 'pre_tool_use',
        toolName: 'exec_command',
