...
function buildMemoryQuery(workItem: WorkItem | null | undefined, globalContext: ContextWindow): string {
  ...
  // Include last 3 explicit user messages from raw context items
  // (avoids file_content/artifact synthetic message leakage)
  const userMessages = globalContext.items
    .filter((item): item is { type: 'message'; role: string; content: string | Array<{ type: string; text?: string }> } =>
