...
export interface AgenticTaskRow {
  ...
  compiled_prompt_path: string | null
...
function rowToAgenticTask(row: AgenticTaskRow): AgenticTask {
  return {
    ...
    compiledPromptPath: row.compiled_prompt_path,
...
export interface AgenticTaskRepository {
  ...
  /** Scheduler entry point: active tasks with compiled prompt, clear circuit, due next_run_at */
  findDueForExecution(limit?: number): Promise<AgenticTask[]>

  /** Transition draft -> active with compiled artifacts */
  activate(id: string, promptPath: string, vpPath: string, vpHash: string): Promise<AgenticTask | null>
  /** Update compiled artifacts (on recompilation) */
  updateCompiled(id: string, promptPath: string, vpPath: string, vpHash: string): Promise<AgenticTask | null>
  ...
  return {
    ...
    async findDueForExecution(limit = 50) {
      const rows = await sql<AgenticTaskRow[]>`
        SELECT * FROM agentic_tasks
        WHERE status = 'active'
          AND compiled_prompt_path IS NOT NULL
    ...
    async activate(id, promptPath, vpPath, vpHash) {
      const [row] = await sql<AgenticTaskRow[]>`
        UPDATE agentic_tasks
        SET status = 'active',
    ...
    async updateCompiled(id, promptPath, vpPath, vpHash) {
      const [row] = await sql<AgenticTaskRow[]>`
        UPDATE agentic_tasks
        SET compiled_prompt_path = ${promptPath},
            compiled_vp_path = ${vpPath},
            compiled_vp_hash = ${vpHash},
            updated_at = NOW()
        WHERE id = ${id}
