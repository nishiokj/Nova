...
from util.config import RouterConfig, LLMConfig
from util.llm_adapter import LLMAdapter, create_adapter, Message, MessageRole
...
class LLMClassifier:
    ...
    def __init__(self, config: LLMConfig, logger: Optional[StructuredLogger] = None):
        ...
        self.adapter = create_adapter(config, logger=self.logger)
...
class Router:
    ...
    def __init__(self, config: RouterConfig, logger: Optional[StructuredLogger] = None):
        self.config = config
        self.logger = logger or StructuredLogger()
        ...
        if config.llm_config:
            self.llm_classifier = LLMClassifier(config.llm_config, logger=self.logger)
