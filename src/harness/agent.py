...
class Agent:
    ...
    def run(
        ...
        try:
            ...
            # Note: trace.steps_executed now returns StepResult objects
            for step_result in trace.steps_executed:
                # Get the corresponding plan step for tool_hint
                plan_step = next((s for s in plan.steps if s.step_num == step_result.step_num), None)

                # Record if there were tool calls in this step
                if step_result.tool_calls_made:
                    # Use the first tool call as representative
                    first_tool = step_result.tool_calls_made[0]
                    agent_step = AgentStep(
                        step_number=self._step_count,
                        state=AgentState.EXECUTING_TOOL,
                        tool_name=first_tool.tool_name,
                        tool_output=first_tool.result.output if first_tool.result.is_success else first_tool.result.error,
