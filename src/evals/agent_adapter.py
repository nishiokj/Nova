...
class AgentExecutionAdapter:
    ...
    def _execute_execution(self, plan: Any, user_prompt: str) -> Dict[str, Any]:
        ...
        try:
            ...
            tools = []
            if hasattr(self.agent, '_get_tool_definitions'):
                tools = self.agent._get_tool_definitions()
            elif hasattr(self.agent, 'tool_registry'):
                tools = self.agent.tool_registry.get_definitions(enabled_only=True)

            trace = executor.execute(
                plan=plan,
                serialized_context=serialized_context,
                tools=tools if plan.requires_tools else []
            )

            duration_ms = (time.time() - start_time) * 1000

            # Extract tool calls for file tracker
            ...
            if not messages:
                # Fall back to synthetic prompts
                messages = self._build_execution_messages(plan, trace, serialized_context)

            # Convert steps
            steps = self._convert_steps(trace.steps if hasattr(trace, 'steps') else [])
            ...
            # Record execution (for HTML export)
            ...
            if self.simple_logger:
                ...
                if exec_calls:
                    ...
                    for i, call in enumerate(exec_calls, 1):
                        exec_response = call.response_content
                        self.simple_logger.log_execution_step(
                            step_number=i,
                            messages=call.messages,
                            tools=call.tools,
                            model=call.model,
                            step_response=exec_response,
                            duration_ms=call.latency_ms
                        )
                else:
                    # Fallback: log with what we have
                    exec_response = self._format_execution_response(trace)
                    self.simple_logger.log_execution_step(
                        step_number=1,
                        messages=messages,
                        tools=tool_schemas,
