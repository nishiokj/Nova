...
class AudioConfig:
    ...
    def __init__(self, config_source: Optional[Union[str, os.PathLike, Dict[str, Any]]] = None):
        """
        Args:
            config_source: Path to config file or a dict with config values.
        """
        if config_source is None:
            config_source = "config/audio_config.json"
        
        if isinstance(config_source, dict):
            self.config_path = None
            self.config = self._merge_with_defaults(config_source)
        elif isinstance(config_source, (str, os.PathLike)):
            self.config_path = os.fspath(config_source)
            self.config = self._load_config()
        else:
            raise TypeError(
                "AudioConfig expects a dict or path-like object for config_source"
            )
    ...
    def _merge_with_defaults(self, config_data: Dict[str, Any]) -> Dict[str, Any]:
        """Merge supplied values with defaults, ignoring unknown keys"""
    ...
    def run(self, device_index: int):
        ...
        try:
            while self.running:
                ...
                # When TTS is speaking, skip audio processing to avoid feedback loops
