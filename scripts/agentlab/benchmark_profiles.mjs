export const BENCHMARK_PROFILES = {
  swebench_lite_curated: {
    aliases: ['swebench-lite', 'swebench_lite', 'swebench_lite_curated'],
    defaultDataset: '.lab/experiments/data/swebench_lite_curated.task_boundary_v2.jsonl',
    sourceDatasetV1: '.lab/experiments/data/swebench_lite_curated.task_boundary_v1.jsonl',
    defaultOutput: '.lab/experiments/swebench_lite_curated_glm5_vs_codex_spark.yaml',
    defaultAgentArtifact: '.lab/agents/rex-current.tar.gz',
    defaultWorkspace: '/testbed',
    timeoutMs: 1_800_000,
    experiment: {
      id: 'swebench_lite_curated_glm5_vs_codex_spark',
      name: 'SWE-bench Lite Curated: GLM-5 vs GPT-5.3 Codex Spark',
      tags: ['swebench-lite', 'curated', 'ab-test', 'glm-5', 'gpt-5.3-codex-spark', 'per-task-image'],
      description: 'A/B evaluation over curated SWE-bench Lite dataset using per-task images.',
    },
    dataset: {
      suiteId: 'swebench_lite_curated',
      schemaVersion: 'task_boundary_v2',
      splitId: 'test',
    },
  },
};

export const DEFAULT_BENCHMARK = 'swebench_lite_curated';
